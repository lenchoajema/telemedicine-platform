...
export default function PatientDashboardPage() {
        ...
        try {
          ...
          try {
            const response = await fetch(
              `${import.meta.env.VITE_API_URL}/patients/recent-doctors`,
              {
                headers: {
          ...
          if (!recentDoctorsRes || recentDoctorsRes.length === 0) {
            try {
              ...
              console.debug(
                "[PatientDashboard] fetched all doctors count:",
                allDoctors.length
              );
              recentDoctorsRes = allDoctors.slice(0, 5).map((d) => ({
                // Prefer real Doctor document id if available
                _id: d.doctorDocId || d.doctorId || d._id || d.userId,
                doctorDocId: d.doctorDocId || d.doctorId || null,
                userId: d.userId,
                profile: {
                  fullName: d.fullName,
              ...
              console.error("Error fetching all doctors (fallback):", error);
            }
          } else {
            console.debug(
              "[PatientDashboard] recentDoctors endpoint count:",
              recentDoctorsRes.length
            );
            recentDoctorsRes = recentDoctorsRes.map((doctor) => ({
              _id: doctor._id, // assumed Doctor document id from endpoint
              doctorDocId: doctor._id,
              userId: doctor.user?._id || doctor.userId,
              profile: {
                fullName:
                  doctor.fullName ||
  ...
  const handleCancelAppointment = async (appointmentId) => {
    try {
      ...
      if (response.ok) {
        ...
        addNotification(
          `Failed to cancel appointment: ${error.error}`,
          "error"
        );
      }
    } catch (error) {
  ...
  const handleJoinCall = (appointment) => {
    if (appointment.meetingUrl) {
      ...
      addNotification(
        `Meeting for your appointment with Dr. ${
          appointment.doctor?.user?.profile?.fullName || "Doctor"
        } will start soon`,
        "info"
      );
            ...
            {recentDoctors.map((doctor) => (
              <div key={doctor._id} className="recent-doctor-card">
                ...
                <div className="doctor-actions">
                  <Link to={`/doctors/${doctor._id}`} className="btn text">
                    View Profile
                  </Link>
                  <Link
                    to={`/appointments/new?doctorId=${
                      doctor.doctorDocId || doctor._id
                    }`}
