  ...
  useEffect(() => {
    const fetchAdminDashboardData = async () => {
      try {
        ...
        const verificationsResponse = await fetch(`${import.meta.env.VITE_API_URL}/admin/verifications/pending`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
        
        if (!verificationsResponse.ok) throw new Error('Failed to fetch pending verifications');
        const verificationsData = await verificationsResponse.json();
        
        setStats(statsData);
        setPendingVerifications(verificationsData);
      } catch (err) {
        addNotification(`Failed to load admin dashboard data: ${err.message}`, 'error');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="dashboard-page admin-dashboard">
      ...
      <section className="dashboard-section">
        <div className="section-header">
          ...
          <Link to="/admin/verifications" className="btn primary">View All</Link>
        </div>
          ...
          <div className="verification-list">
            ...
            {pendingVerifications.length > 5 && (
              <div className="view-more">
                <Link to="/admin/verifications">View all {pendingVerifications.length} pending verifications</Link>
              </div>
            )}
          </div>
        )}
