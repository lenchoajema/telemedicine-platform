exports.handler = async function (event, context) {
  ...
  if (!backend) {
    return { statusCode: 501, body: JSON.stringify({ error: 'Not implemented in serverless fallback', route: '/api/admin' }) };
  }
  try {
    const suffix = event.path.replace('/.netlify/functions/api/generated/admin', '') || '/';
    const url = new URL('/api/admin' + suffix, backend);
